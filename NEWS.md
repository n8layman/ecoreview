...
- **OCR viewer shows images** (#38): OCR image placeholders such as `![img-0.jpeg](img-0.jpeg)` rendered as broken images. `render_tensorlake_html()` gains `ocr_images`, and the app passes the stored `documents.ocr_images`, so each page's placeholders are replaced with the stored image of the same `id`. Images are matched by id rather than position because Mistral numbers image ids across the whole document (a page's first image is often not `img-0`). Placeholders with no stored image show as muted text instead of a broken icon. On 29 LEMIS shipments all 83 images embed correctly; evidence matching is unchanged.
