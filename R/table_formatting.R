...
create_styled_datatable <- function(data, height = "600px",
  ...
  DT::datatable(data,
    ...
    options = list(
      ...
      columnDefs = list(list(
        ...
        createdCell = DT::JS(
          ...
          "  if (text.length > 0) {",
          "    var sentences = null;",
          "    try { var p = JSON.parse(text); if (Array.isArray(p)) sentences = p; } catch(e) {}",
          "    var timer;",
          "    $(td).on('mouseenter', function(e) {",
          "      timer = setTimeout(function() {",
          "        if (sentences !== null) {",
          "          var unmatched = (typeof ecrUnmatchedByRow !== 'undefined' && ecrUnmatchedByRow[String(row)]) ? ecrUnmatchedByRow[String(row)] : [];",
          "          var unmatchedSet = {};",
          "          unmatched.forEach(function(s) { unmatchedSet[String(s)] = true; });",
          "          var html = '<ol style=\"margin:4px 0;padding-left:18px;\">';",
          "          sentences.forEach(function(s) {",
          "            var esc = String(s).replace(/&/g,'&amp;').replace(/</g,'&lt;').replace(/>/g,'&gt;');",
          "            if (unmatchedSet[String(s)]) {",
          "              html += '<li style=\"color:#ccc;font-style:italic\"><span style=\"color:#e09800\" title=\"Not found in OCR text\">\u26a0\ufe0f</span>\u00a0' + esc + '</li>';",
          "            } else {",
          "              html += '<li>' + esc + '</li>';",
          "            }",
          "          });",
          "          html += '</ol>';",
          "          $('#dt-tooltip').html(html).css({display:'block', left: e.pageX+12, top: e.pageY+12});",
          "        } else {",
    ...
    callback = DT::JS(
      ...
      "  $('body').append('<div id=\"dt-tooltip\" style=\"position:fixed;background:#333;color:#fff;padding:6px 10px;border-radius:4px;font-size:12px;max-width:420px;word-wrap:break-word;z-index:9999;pointer-events:none;display:none;\"></div>');",
      "}",
